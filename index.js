(async () => {
  ...
  if (process.env.TRACKER_CONFIG_FILE)
    var [err, track] = await __.to(
      fsPromise.readFile(process.env.TRACKER_CONFIG_FILE)
    );
  if (!track && process.env.TRACKER_SECRET_FILE)
    var [err, track] = await __.to(
      fsPromise.readFile("/run/secrets/" + process.env.TRACKER_SECRET_FILE)
    );

  var embed = "";
  if (process.env.EMBED_FILE)
    var [err, embed] = await __.to(fsPromise.readFile(process.env.EMBED_FILE));
  ...
  if (DEBUG)
    console.log(
      "content prepeared",
      "+" + ((new Date().getTime() - tsStart) / 1000).toFixed(2) + " sec"
    );
  ...
  const server = http.createServer(async (request, response) => {
    ...
    let queryLang =
      query.lang && (query.lang == "ru" || query.lang == "en")
        ? query.lang
        : false;
